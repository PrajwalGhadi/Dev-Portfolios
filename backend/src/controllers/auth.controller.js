...
async function register(req, res) {
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ success: false, errors: [{ message: "User already exists" }] });
    }
    ...
    res
      .status(201)
      .json({ success: true, errors: [{ message: "User registered successfully" }] });
  } catch (error) {
...
async function login(req, res) {
  try {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .json({ success: false, errors: [{ message: "Email and password are required" }] });
    }
    ...
    res
      .status(500)
      .json({ success: false, errors: [{ message: `Login Failed: ${error.message}` }] });
  }
}
...
async function logout(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: `Logout Failed: ${error.message}` });
  }
}
...
async function updateUser(req, res) {
  try {
    ...
    const updatedUser = await userModel
      .findByIdAndUpdate(req.user_id, updatedData, { new: true })
      .select("-password");
